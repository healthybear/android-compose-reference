...
<template>
  <el-header class="flex items-center justify-between gap-2 md:gap-4 w-full">
    <div class="flex items-center gap-2 flex-shrink-0">
      <el-button :icon="isMobile ? (drawerOpen ? Close : Expand) : (collapsed ? Expand : Fold)" text circle @click="emit('toggleSidebar')" />
    ...
    <div class="flex-shrink-0"><el-button :icon="isDark ? Sunny : Moon" circle @click="emit('toggleTheme')" /></div>
  </el-header>
</template>
